...
describe('server responses', () => {
  ...
  it('should respond to a GET request for a swim command', (done) => {
    ...
    const randomCheck = function(num) {
      const results = [];
      for (let i = 0; i <= num; i++) {
        let { res } = server.mock('http://127.0.0.1:3000/getmove', 'GET');
        keyPressHandler.randomMove(res);
        const move = Object.values(JSON.parse(res._data))[0];
        results.push(move);
      }
      return results.every(function(n) {
        n === results[0];
      });
    }
    let { req, res } = server.mock('http://127.0.0.1:3000/getmove', 'GET');
    keyPressHandler.randomMove(res);

    httpHandler.router(req, res, () => {
      const move = Object.values(JSON.parse(res._data))[0];
      expect(move).to.be.a('string');
      expect(keyPressHandler.isValidMessage(move)).to.equal(true);
      randomCheck(5);
      done();
    });
  });
  ...
  it('should respond with 404 to a GET request for a missing background image', (done) => {
    ...
    let { req, res } = server.mock('http://127.0.0.1:3000/getbackground', 'GET');
